...
async function proxy(req: NextRequest, path: string[]) {
  ...
  console.log('🍪 Proxy cookie header:', cookieHeader ? cookieHeader.slice(0, 120) + '...' : 'NONE')
  ...
  try {
    ...
    console.log('⬅️ Backend status:', backendRes.status, 'for', targetUrl)
    ...
    return NextResponse.json({ error: 'Proxy failed', details: errorMessage }, { status: 502 })
  }
}
